    ...
    cpdef void setup(self, LinearOperator A=None):
        ...
        if isinstance(B, Dense_LinearOperator):
            ...
            IF USE_CHOLMOD:
                ...
                if isinstance(B, CSR_LinearOperator):
                    try:
                        self.Ainv = cholesky(B.to_csc())
                    except AttributeError:
                        self.Ainv = cholesky(B.to_csr().tocsc())
                elif isinstance(B, SSS_LinearOperator):
                    self.Ainv = cholesky(B.to_lower_csc())
                else:
                    try:
                        B = B.to_csr_linear_operator()
                        self.Ainv = cholesky(B)
                    except AttributeError:
                        raise NotImplementedError()
            ELSE:
